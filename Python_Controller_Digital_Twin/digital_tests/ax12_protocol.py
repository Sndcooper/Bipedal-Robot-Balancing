...
def ik_angle_to_ax12(ik_deg, offset=150.0, invert=False):
    ...
    # Wrap angle so it's continuous around the AX-12 center (150 degrees)
    # This completely prevents 360-degree leaps when IK wraps from -180 to +180
    diff = (ax_deg - 150.0)
...
def build_read_packet(servo_id, address, length):
    """Build READ packet to read `length` bytes starting at `address`."""
    return build_packet(servo_id, 0x02, [address, length])


def build_write_packet(servo_id, address, value, num_bytes=None):
    """Build WRITE packet for 1 or 2 byte value.
    If num_bytes is provided, it forces the length of the data."""
    if isinstance(value, int):
        if num_bytes == 1 or (num_bytes is None and value <= 0xFF):
            # Single byte
            return build_packet(servo_id, 0x03, [address, value & 0xFF])
        else:
...
class AX12Controller:
    ...
    def _connect(self):
        ...
        try:
            ...
            time.sleep(2.5)  # Wait for STM32 to boot and stabilize
            ...
            if self.ser.in_waiting:
                ...
                if self.debug:
                    try:
                        lines = startup_data.decode('utf-8', errors='ignore').strip().split('\n')
                        for line in lines:
                            if line.strip():
            ...
            if self.debug:
                ...
                print(f"[AX12] STM32 is in RAW passthrough mode")
        ...
        if self.debug:
            if response:
                ...
                if len(response) >= 5:
                    ...
                    # Show ID mismatch warning
                    if recv_id != sent_id:
                        print(f"RX: {hex_str} -> ERROR: ID MISMATCH (sent to {sent_id}, got reply from {recv_id})")
                    elif error == 0:
                        print(f"RX: {hex_str} -> OK")
                    else:
                        error_bits = bin(error)[2:].zfill(8)
                        print(f"RX: {hex_str} -> ERROR: 0b{error_bits}")
                        self._decode_error(error)
                else:
                    print(f"RX: {hex_str} -> Incomplete response")
            else:
    ...
    def read_register(self, servo_id, address, length):
        """Read arbitrary register. Returns list of bytes or None."""
        packet = build_read_packet(servo_id, address, length)
        response = self._send_packet(packet)
        if response and len(response) >= 5 + length:
            error = response[4]
            if error == 0:
                payload = response[5 : 5+length]
                return list(payload)
            else:
                if self.debug: self._decode_error(error)
        return None

    def get_load(self, servo_id):
        """Get present load. Returns float from -100.0 (CCW max) to 100.0 (CW max)."""
        data = self.read_register(servo_id, AX12_ADDR.PRESENT_LOAD, 2)
        if data:
            val = data[0] | (data[1] << 8)
            # Bit 10 determines direction. 1 = CCW, 0 = CW
            direction = -1 if (val & 0x400) else 1
            load_raw = val & 0x3FF
            return (load_raw / 1023.0) * 100.0 * direction
        return None

    def get_position(self, servo_id):
        """Get present position. Returns integer (0-1023) or None."""
        data = self.read_register(servo_id, AX12_ADDR.PRESENT_POSITION, 2)
        if data:
            val = data[0] | (data[1] << 8)
            return val
        return None

    def set_angle_limits(self, servo_id, cw_limit, ccw_limit):
        """Set CW (min) and CCW (max) angle limits (0-1023)."""
        cw_limit = max(0, min(1023, int(cw_limit)))
        ccw_limit = max(0, min(1023, int(ccw_limit)))
        # Angle limits are always 2 bytes
        packet_cw = build_write_packet(servo_id, AX12_ADDR.CW_ANGLE_LIMIT, cw_limit, num_bytes=2)
        packet_ccw = build_write_packet(servo_id, AX12_ADDR.CCW_ANGLE_LIMIT, ccw_limit, num_bytes=2)
        self._send_packet(packet_cw)
        time.sleep(0.05)
        self._send_packet(packet_ccw)
        if self.debug:
            print(f"[LIMITS] Servo {servo_id} limits set to {cw_limit} (CW) - {ccw_limit} (CCW)")

    def set_position(self, servo_id, position):
        """Set goal position (0-1023)."""
        position = max(0, min(1023, int(position)))
        packet = build_write_packet(servo_id, AX12_ADDR.GOAL_POSITION, position, num_bytes=2)
        self._send_packet(packet)
    
    def set_speed(self, servo_id, speed):
        """Set moving speed (0-1023). 0 = max speed."""
        speed = max(0, min(1023, int(speed)))
        packet = build_write_packet(servo_id, AX12_ADDR.MOVING_SPEED, speed, num_bytes=2)
        self._send_packet(packet)
    
    def set_torque_limit(self, servo_id, torque):
        """Set torque limit (0-1023)."""
        torque = max(0, min(1023, int(torque)))
        packet = build_write_packet(servo_id, AX12_ADDR.TORQUE_LIMIT, torque, num_bytes=2)
        self._send_packet(packet)
    ...
    def ping(self, servo_id):
        ...
        """
        packet = build_packet(servo_id, 0x01, [])  # PING instruction
        response = self._send_packet(packet)
        
        if response and len(response) >= 4:
            # Valid response received
            if self.debug:
                print(f"[PING] Servo ID {servo_id} is online")
            return True
        else:
            if self.debug:
                print(f"[PING] Servo ID {servo_id} did not respond")
            return False
