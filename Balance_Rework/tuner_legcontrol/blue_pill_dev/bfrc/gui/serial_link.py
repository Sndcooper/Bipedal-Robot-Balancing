...
class SerialLink:
    def __init__(self, port="COM13", baud=115200):
        ...
        self.rc = {
            ...
            "max_steer":  40.0,
            "max_crouch": 40.0,
            # Ch8 accumulated target and its rate band
            "target":      0.0,
            "rate_min":   10.0,
            "rate_max":  100.0,
            # Ch9 leg selector: 0=mirrored, 1=left only, 2=right only
            "leg_sel":       0,
            # Ch10 decoded crouch axis (-1..+1), rate command not a position
            "crouch_axis": 0.0,
            "crouch_rate": 25.0,
            # Per-channel calibration, Ch1..Ch10. Mirrors the firmware's table;
            # populated from the robot by RCD (see _parse_rc_cal), never assumed
            # to match a local file.
            "cal": [{"min": 1000, "cen": 1500, "max": 2000} for _ in range(10)],
        }

        # Loop-body timing, reported one tick late by the firmware (see the
        # LOOP-BODY TIMER block in main.cpp for why).
        self.loop_timing = {
            "body_us":   0,   # previous tick's body time
            "period_us": 0,   # previous tick's start-to-start interval
            "peak_us":   0,   # worst body time since the last LTR reset
            "overruns":  0,   # ticks whose body exceeded 10000 us
        }
    ...
    def _process_line(self, line: str):
        ...
        clean_line = line
        for prefix in ("PITCH:", "AX12:", "SRV:", "RCCAL:", "RC:", "Updated ->", "BOOT:",
    ...
    def _parse_telemetry(self, line):
        ...
        with self._lock:
            ...
            if "RCT"  in data:
                self.rc["target"]      = data["RCT"]
            if "RCG"  in data:
                self.rc["leg_sel"]     = int(data["RCG"])
            if "RCC"  in data:
                self.rc["crouch_axis"] = data["RCC"]

            # Loop-body timing. LUS is the PREVIOUS tick's body time - the
            # firmware defers it by one tick so the measurement window closes
            # before the telemetry write that reports it.
            if "LUS"  in data:
                self.loop_timing["body_us"]   = int(data["LUS"])
            if "LPER" in data:
                self.loop_timing["period_us"] = int(data["LPER"])
            if "LPK"  in data:
                self.loop_timing["peak_us"]   = int(data["LPK"])
            if "LOVR" in data:
                self.loop_timing["overruns"]  = int(data["LOVR"])
    ...
    def _parse_rc_channels(self, line):
        """Parses: RC:1500,1500,...,LINK:1

        TEN raw channel widths in microseconds (Ch1..Ch10) then the link flag.
        Ten, not eight, because this variant's map uses Ch9 (leg select) and
        Ch10 (crouch); a parser that stops at eight silently drops both.
    ...
    def _parse_rc_cal(self, line):
        """Parses one line of the firmware's calibration dump.

            RCCAL:<ch>,<min>,<cen>,<max>

        <ch> is 1-BASED, matching the transmitter's labelling and the GUI; it is
        converted to a 0-based index here. Emitted once per channel in response
        to RCD, so the GUI shows what the robot is actually using rather than a
        local file that may have drifted from the flashed firmware.
        """
        try:
            _, payload = line.split(":", 1)
            parts = [int(float(x)) for x in payload.split(",")[:4]]
            if len(parts) != 4:
                return
            ch, mn, cn, mx = parts
            if not (1 <= ch <= 10):
                return
            with self._lock:
                self.rc["cal"][ch - 1] = {"min": mn, "cen": cn, "max": mx}
        except (ValueError, IndexError):
            pass
    ...
    def get_rc_state(self):
        with self._lock:
            ...
            # dict(self.rc) is shallow, so "cal" would alias the live list of
            # dicts and let a UI edit mutate parser state without the lock.
            state["cal"] = [dict(c) for c in self.rc["cal"]]
            return state

    def get_loop_timing(self):
        with self._lock:
            return dict(self.loop_timing)
    ...
    # ch is 1-BASED here and on the wire; the firmware converts to its own
    # 0-based index. Keeping the 1-based convention end-to-end means the number
    # in the GUI, the number on the wire and the number on the transmitter all
    # agree, and the single conversion lives in one place in main.cpp.
    def set_rc_cal(self, ch, mn, cen, mx):
        self._send(f"RCC{int(ch)},{int(mn)},{int(cen)},{int(mx)}")

    def request_rc_cal(self):  self._send("RCD")
    def reset_rc_cal(self):    self._send("RCZ")
    def reset_loop_timer(self): self._send("LTR")
