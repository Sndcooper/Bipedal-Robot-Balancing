...
// ----------------------------------------------------------------------------
// Channel map for THIS variant (blue_pill_dev/balance_fusion_rc). It is NOT the
// map of rc_balance_fusion_wireless and NOT the map of RC_mcu_IK_wireless — all
// three differ. readChannel() is 0-indexed, so Ch<n> is readChannel(n-1).
//
//   Ch3  (idx2) DRIVE     : self-centring pot, fwd/back. Velocity target for
//                           the outer (velocity->lean) loop. Idle -> 0, i.e.
//                           the robot simply balances in place.
//   Ch4  (idx3) STEER     : self-centring pot, left/right. Differential PWM
//                           trim applied AFTER the balance PID. Idle -> 0.
//   Ch5  (idx4) CALIBRATE : momentary button. Rising edge starts an IMU cal.
//   Ch6  (idx5) ZERO TGT  : momentary button. Rising edge zeroes the target
//                           (target_velocity, integral, trim_bias, encoders).
//   Ch7  (idx6) MOTORS    : 2-position switch. HIGH = armed, LOW = disarmed.
//   Ch8  (idx7) TARGET ±  : self-centring pot. INTEGRATES a position target at
//                           up to RC_TARGET_RATE counts/sec (see below) — this
//                           is a rate control, not a direct setpoint.
//   Ch9  (idx8) LEG SEL   : 3-position switch selecting what Ch10 moves:
//                           LOW = mirrored (both legs), MID = left leg only,
//                           HIGH = right leg only.
//   Ch10 (idx9) CROUCH    : pot. Centre = hold. Off-centre crouches (+) or
//                           stretches (-) the leg(s) Ch9 selected, as a RATE.
//
// Ch1/Ch2 are deliberately unused here so a mode-2 TX's right-hand stick can be
// left alone; drive/steer moved to the Ch3/Ch4 pots at the operator's request.
// ============================================================================
IBusBM ibus;
...
uint32_t rcRxBytes = 0;      // DIAG: raw USART1 bytes seen
...
// Raw values for all TEN channels, forwarded to the GUI so the operator can
// verify the channel map and calibrate endpoints without a TX-side display.
// Widened from 8 to 10 because this map uses Ch9 and Ch10.
#define RC_NUM_CH 10
uint16_t rc_raw[RC_NUM_CH] = {0};

// ── PER-CHANNEL CALIBRATION (min / centre / max, in microseconds) ───────────
// The GUI's calibration tab captures these per channel and writes them back
// with RCC<ch>,<min>,<centre>,<max>. Defaults are the nominal FlySky endpoints,
// so an uncalibrated robot behaves exactly as it did before. Stored for all ten
// channels even though the switches only use the centre value as a threshold.
...
// ── Ch8 TARGET-RATE control ────────────────────────────────────────────────
// The pot does NOT set the target directly; it sets the RATE at which the
// target moves, and returns to idle (no change) when centred. Requested
// behaviour: "changes target by 10-100 slowly not fast, rate of 100 per sec at
// max, and at low pot step slowly".
//
// So the axis maps onto a rate band, not a linear 0..max:
//   just off centre -> RC_TARGET_RATE_MIN (10 counts/sec)
...
// Same idea: pot centred = hold the current crouch, off-centre = move at a
// rate. A rate control is the right primitive here because the pot has no
// mechanical detents matching leg geometry, and because startPoseMove() must
// not be restarted on every poll (see readRC()).
float RC_CROUCH_RATE = 25.0f;        // mm/sec at full Ch10 pot     — RCR cmd

// ============================================================================
// LOOP-BODY TIMER - how long the tick ACTUALLY took
// ----------------------------------------------------------------------------
// The requirement: capture the time from the start of the loop body to AFTER
// its last task, and report that number on the FOLLOWING tick.
//
// Why deferred by one tick, and why that is the correct design rather than a
// workaround: the only place the measurement can be *finished* is after the
// last task, but the telemetry write is itself one of the tasks. Printing the
// number inside the same tick would mean either
//   (a) printing before the body ends - measuring an incomplete tick, or
//   (b) printing after the body ends - and the print, which is the single most
//       expensive thing in the tick (a ~200 byte snprintf plus a UART write),
//       then lands OUTSIDE the window it is reporting on.
// Both are the same bug: the number stops describing the work it claims to.
// Stashing the finished figure in loop_us_last and emitting it on the next
// tick's telemetry keeps the measured window closed and complete, at the cost
// of the reported value being one tick (10 ms) old - which does not matter for
// a budget figure.
//
// loop_us_body is the body cost; loop_us_period is start-to-start, i.e. the
// body plus whatever idle remained. On a healthy 100 Hz loop period is ~10000
// and body is well under it; body creeping toward 10000 is the overrun warning.
uint16_t loop_us_last   = 0;     // previous tick's body time, reported now
uint16_t loop_us_period = 0;     // previous tick's start-to-start interval
uint16_t loop_us_peak   = 0;     // worst body time since the last RESET (LTR)
uint32_t loop_overruns  = 0;     // ticks whose body exceeded 10000 us
...
void parseCommand(char *cmd) {
  ...
  // <ch> is 1-based (matching the transmitter's own labelling and the GUI), so
  // it is converted to the 0-based array index here - the single most likely
  ...
  // A bad line is rejected with a NAK and the previous calibration survives.
  else if (cmd[0]=='R' && cmd[1]=='C' && cmd[2]=='C') {
    int ch = 0; int mn = 0, cn = 0, mx = 0;
  ...
  // can populate its calibration tab from the robot rather than from a local
  // file that may not match what is actually flashed.
  ...
  // -- Ch8 target-rate band -------------------------------------------------
  // RTN<f> slow end, RTX<f> fast end, RTL<f> clamp on the accumulated target.
  ...
  // pot move the target FASTER the closer it sits to centre.
    ...
    RC_TARGET_RATE_MAX = constrain(atof(cmd + 3), RC_TARGET_RATE_MIN, 2000.0f);
  }
  else if (cmd[0]=='R' && cmd[1]=='T' && cmd[2]=='L') {
    RC_TARGET_LIMIT = constrain(atof(cmd + 3), 0.0f, 100000.0f);
  }

  // LTR - reset the loop-timer peak and overrun counter, so a tuning change
  // can be judged on a fresh window instead of a peak set during boot.
  else if (cmd[0]=='L' && cmd[1]=='T' && cmd[2]=='R') {
    loop_us_peak  = 0;
    loop_overruns = 0;
    snprintf(ack, sizeof(ack), "ACK:LOOPTIMER_RESET");
  }
...
// Map a raw iBUS pulse width to -1..+1 using that channel's CALIBRATION, with
// a centre deadband. Returns exactly 0.0 inside the deadband and for a dead
// channel (raw 0), so a disconnected or unbound channel is indistinguishable
// from a centred stick - the safe failure, not a stuck full-deflection command.
//
// The two sides are scaled INDEPENDENTLY against their own endpoint, because a
// calibrated pot is rarely symmetric: centre might sit at 1480 with travel to
// 1000 and 2000, so the low side has 480 us of travel and the high side 520.
// Scaling both by one span would make full-left read 0.92 while full-right read
// 1.0. Per-side scaling makes both ends reach exactly 1.0.
static float rcAxisCal(uint16_t raw, uint8_t ch) {
  if (raw == 0) return 0.0f;                       // channel never populated
...
// Unipolar 0..1 read (pot travelling one way only), calibration-aware.
...
// Ch10, both of which are rate controls: idle at centre, slow just off centre,
// fastest at the stop. Squaring expands the fine end of the pot's travel, which
// is where an operator does the careful work.
...
void readRC() {
  ...
  // NO DECIMATION - ibus.loop() MUST run every tick.
  //
  // This used to be gated to one call per RC_POLL_INTERVAL_MS (10 ms):
  //     if (nowMs - rcLastPollMs < RC_POLL_INTERVAL_MS) return;
  // That silently broke decoding completely, and the failure mode is worth
  // stating because nothing about it is visible from the outside:
  //
  //   IBusBM::loop() resynchronises on a GAP. Its first action per byte is
  //     if (millis() - last >= PROTOCOL_TIMEGAP) state = GET_LENGTH;
  //   with PROTOCOL_TIMEGAP = 3 ms, whose own comment reads "packets are
  //   received every ~7ms so use ~half that for the gap". The library ASSUMES
  //   it is called far more often than a frame arrives.
  //
  //   Calling it every 10 ms guarantees that gap is ALWAYS exceeded. So on
  //   every call the decoder discarded its state and treated whatever byte
  //   came next as a frame-LENGTH byte - mid-frame, at an arbitrary offset.
  //   It could never assemble a whole 32-byte frame, cnt_rec never
  //   incremented, every channel read 0, and the failsafe reported LINK:0
  //   forever. The receiver, wiring and transmitter were all fine.
  //
  // The decoder is a cheap byte-at-a-time state machine over a buffer already
  // in RAM, so running it on every 10 ms tick costs almost nothing, and the
  // microsecond budget below still bounds it. RC_POLL_INTERVAL_MS is retained
  // ONLY as the rate-integration timebase (Ch8/Ch10), not as a poll gate.
  rcLastPollMs = nowMs;
  ...
  // DIAG: count raw bytes the UART delivered, BEFORE IBusBM consumes them.
  // Separates "nothing arriving on PA10" (rcRxBytes stays 0) from "bytes
  // arrive but do not decode" (rcRxBytes climbs, cnt_rec does not) - a
  // distinction the channel values cannot make, since both read 0.
  rcRxBytes += (uint32_t)Serial1.available();
  ...
  for (uint8_t i = 0; i < RC_NUM_CH; i++) rc_raw[i] = ibus.readChannel(i);
  ...
  // robot just balances in place. That is the requested "when idle just
  // balances normally" behaviour and it falls out of the deadband, with no
  // special-casing needed.
  rc_drive = rcAxisCal(rc_raw[2], 2);   // Ch3, front/back
  rc_steer = rcAxisCal(rc_raw[3], 3);   // Ch4, left/right

  // -- Ch8: target +/- as a RATE, integrated here --------------------------
  // Integrated against the ACTUAL poll interval rather than a nominal constant,
  // so changing RC_POLL_INTERVAL_MS cannot silently change how fast the target
  // slews. First poll after a gap uses the nominal interval to avoid a jump.
  ...
  // pushed before arming must not bank a setpoint that takes effect the instant
  // the motors come on.
  ...
  // -- Ch6: set target to 0 (momentary button, rising edge) ----------------
  // Clears the accumulated Ch8 target AND the state that would otherwise carry
  // the old target forward: the velocity setpoint, the outer loop's learned
  // trim, and the encoder origin. Safe to press while balancing - it commands
  // "stop here", which is the whole point of the button.
  if (rc_raw[5] > 0) {
  ...
  // Thresholds are derived from the channel's own calibration so a TX whose
  // 3-pos switch does not sit at 1000/1500/2000 still resolves correctly: the
  // two boundaries are placed a quarter and three quarters across the travel.
  if (rc_raw[8] > 0) {
  ...
  // Centre = hold (does nothing, as requested). Off-centre moves the selected
  // leg(s) at a rate, positive = crouch, negative = stretch.
  //
  // Why a rate and not a direct pot->depth map: startPoseMove() RESTARTS the
  // interpolation every time it is called, so driving it from a jittering pot
...
void loop() {
  ...
  // Start-to-start interval of the PREVIOUS tick, captured before lastTime is
  // overwritten. This is the period; the body time is measured separately
  // below. Both are reported together so a tick that is late (long period) can
  // be told apart from a tick that is slow (long body).
  loop_us_period = (uint16_t)min<unsigned long>(now - lastTime, 65535UL);
  lastTime = now;

  // The body window opens here. It closes at the very bottom of loop(), after
  // the last task - see the LOOP-BODY TIMER block above for why the result is
  // reported on the following tick rather than this one.
  ...
  if (now - lastPrintTime >= 100000) {
    ...
    // 384, not 256. The PITCH line carries 34 fields and measures ~288 chars
    // typically, ~340 worst case (large encoder counts, LOVR near UINT32_MAX).
    //
    // WHY THIS MATTERS MORE THAN IT LOOKS: snprintf() TRUNCATES to the buffer
    // but RETURNS THE LENGTH IT WOULD HAVE WRITTEN. So an undersized buffer
    // does not produce a short line - it makes the `n < sizeof(line)` guard
    // below false on EVERY tick, and the line is dropped silently, forever.
    // The AX12/SRV/RC lines share this buffer but are far shorter, so they
    // keep flowing and the link looks perfectly healthy while the main
    // telemetry never arrives. Anything added to this line must be measured
    // against this size.
    //
    // Kept <= SERIAL_TX_BUFFER_SIZE (384) so availableForWrite() can still
    // pass for a whole line.
    char line[384];
    int n = snprintf(line, sizeof(line),
      ...
      "RCT:%.1f,RCG:%u,RCC:%.2f,LUS:%u,LPER:%u,LPK:%u,LOVR:%lu\n",
      ...
      rc_target, (unsigned)rc_legSel, rc_crouchAxis,
      (unsigned)loop_us_last, (unsigned)loop_us_period,
      (unsigned)loop_us_peak, (unsigned long)loop_overruns);
    if (n > 0 && n < (int)sizeof(line) && Serial3.availableForWrite() >= n) {
      Serial3.write((uint8_t*)line, n);
    } else if (n >= (int)sizeof(line)) {
      // The line no longer fits. Say so, at 1 Hz, instead of vanishing:
      // a silent drop here cost a full debugging session once already.
      static uint8_t ovfDiv = 0;
      if (++ovfDiv >= 10) {
        ovfDiv = 0;
        char w[48];
        int m = snprintf(w, sizeof(w), "ERR:TELEM_TRUNC %d/%u\n",
                         n, (unsigned)sizeof(line));
        if (m > 0 && Serial3.availableForWrite() >= m)
          Serial3.write((uint8_t*)w, m);
      }
    }
    ...
    if (++rcStateDiv >= 5) {
      ...
      int r = snprintf(line, sizeof(line),
        "RC:%u,%u,%u,%u,%u,%u,%u,%u,%u,%u,LINK:%d,RX:%lu,FR:%u\n",
        ...
        (unsigned long)rcRxBytes, (unsigned)ibus.cnt_rec);
  ...
  // -- CLOSE THE LOOP-BODY WINDOW ------------------------------------------
  // Last statement in loop(), so everything above is inside the measurement.
  // The value is stashed, NOT printed: the next tick's telemetry reports it.
  // Anything added to this function must go ABOVE this line or it escapes the
  // measurement entirely.
  unsigned long body = micros() - loop_t0;
  loop_us_last = (uint16_t)min<unsigned long>(body, 65535UL);
  if (loop_us_last > loop_us_peak) loop_us_peak = loop_us_last;
  if (body > 10000UL) loop_overruns++;
}
