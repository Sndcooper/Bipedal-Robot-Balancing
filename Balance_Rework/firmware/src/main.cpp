...
//               servo that silently drops Torque Enable (overload trip,
//               brownout, EEPROM Max Torque mismatch, etc.) gets re-asserted
//               automatically instead of staying limp until the next manual "S"
//               reset.
...
float Kp = 11.21, Ki = 0.0,
      Kd = 0.715; // starting point from the old "tuned" run
float targetAngle = 0.0;
float integral = 0.0, prevError = 0.0;
unsigned long lastTime = 0;
unsigned long lastPrintTime = 0;

// --- NEW tunable globals (all live-tunable over serial, see
// handleSerialTuning) --- Complementary filter blend factor: pitch =
// alpha*(gyro-integrated) + (1-alpha)*accel. Higher alpha => trust the gyro
// more (smoother, but drifts); lower => trust accel more.
float alpha = 0.96;

// Encoder velocity damping gain. Subtracts Kd_vel * wheelVelocity from the PID
// output to fight the "run away then lurch back" wobble. Default 0.0 => feature
// off until swept.
float Kd_vel = 0.0;

// Safety tilt limit (degrees). Default seeded from the #define below;
// live-tunable via "T".
#define MAX_SAFE_TILT_DEFAULT 35.0f
float maxSafeTilt = MAX_SAFE_TILT_DEFAULT;

// Sign of the gyro-Y contribution in the complementary filter. Depends on how
// the MPU is physically mounted. If, when you slowly tilt the robot by hand on
// the harness, the fused PITCH lags or runs AWAY from the raw accelerometer
// tilt instead of tracking it, flip this to -1.0f and reflash. (See
// Balance_Rework/README.md, "First-run checklist".)
#define GYRO_PITCH_SIGN 1.0f

// Raw accelerometer-only pitch (before offset subtraction), captured each
// readIMU(). Used by calibrateIMU() so calibration never depends on the fused
// estimate.
float accelPitchRaw = 0.0;

bool motorsEnabled = false; // safe toggle
bool safetyLatched =
    false; // set true when a safety cutoff has latched motors off
...
void initAX12Legs() {
  ...
  // Upright standing pose was: left_positions[] = {818, 818}, right_positions[]
  // = {441, 441} left_servos = {6, 0}; right_servos = {14, 1}.
  ...
  for (int i = 0; i < 2; i++) {
    for (uint8_t id : {left_servos[i], right_servos[i]}) {
      ...
      ax12WriteByte(id, 29,
                    4); // CCW Compliance Slope (stiffer holding, was 16)
    }
...
// Periodically re-asserts Torque Enable, Torque Limit, and Goal Position for
// one leg servo at a time. Staggered (one servo per HEAL_INTERVAL_MS) rather
// than all four at once, so we're not hammering the power rail with 4
// simultaneous torque re-enables every cycle. This does NOT read anything back
// from the servos (your half-duplex wiring hasn't been confirmed to support
// clean reads yet) — it just blindly re-asserts state, which is enough to
// recover from a silent Torque Enable drop (overload trip / brownout / EEPROM
// Max Torque mismatch) without needing to know why it happened.
//
...
uint8_t healIndex = 0;
const unsigned long HEAL_INTERVAL_MS =
    1500; // one servo every 1500ms -> full sweep every 6s
...
void readIMU(float dt) {
  ...
  accelPitchRaw =
      atan2((float)-ax, sqrt((float)ay * (float)ay + (float)az * (float)az)) *
      180.0 / PI;
  float accelPitch = accelPitchRaw - pitchOffset;

  // Gyro Y rate in deg/s (MPU6050 default full-scale ±250 dps => 131
  // LSB/(deg/s)).
  float gyroRate = GYRO_PITCH_SIGN * (float)gy / 131.0;

  // Complementary filter: integrate the gyro for the fast/smooth term, correct
  // the slow drift with the accelerometer.
...
void handleSerialTuning() {
  if (Serial1.available()) {
    ...
    } else if (input.startsWith("S") && input.length() > 1)
      targetAngle = input.substring(1).toFloat();
    else if (input.startsWith("V"))
      Kd_vel = input.substring(1).toFloat(); // NEW: velocity damping gain
    else if (input.startsWith("A"))
      alpha = input.substring(1).toFloat(); // NEW: complementary filter blend
    else if (input.startsWith("T"))
      maxSafeTilt = input.substring(1).toFloat(); // NEW: safety tilt limit
    else if (input.startsWith("C"))
      calibrateIMU();
    else if (input.startsWith("M")) {
      motorsEnabled = !motorsEnabled;
      if (motorsEnabled) {
        // Explicit re-enable clears the safety latch and resets controller
        // state so a prior fall doesn't kick the motors on re-enable.
        safetyLatched = false;
        integral = 0.0;
      }
      Serial1.print("Motors ");
      Serial1.println(motorsEnabled ? "ENABLED" : "DISABLED");
    }

    // Extended status print — original P/I/D/Offset/Target fields preserved,
    // new Vel/Alpha/Tilt fields appended.
    Serial1.print("Updated -> P:");
    Serial1.print(Kp);
    Serial1.print(" I:");
    Serial1.print(Ki);
    Serial1.print(" D:");
    ...
    Serial1.print(pitchOffset);
    Serial1.print(" Target:");
    Serial1.print(targetAngle);
    Serial1.print(" Vel:");
    Serial1.print(Kd_vel, 4);
    Serial1.print(" Alpha:");
    Serial1.print(alpha, 4);
    Serial1.print(" Tilt:");
    Serial1.println(maxSafeTilt);
  }
}
...
void setup() {
  ...
  // 2-second delay to allow AX-12+ servos to power up and stabilize before
  // initializing Serial2. This leaves the UART pins floating/high-impedance
  // during the servo boot sequence to prevent noise.
  delay(2000);
  ...
  lastHealTime = millis(); // start the leg heartbeat timer fresh so it doesn't
                           // fire immediately
}
...
void loop() {
  ...
  // ── SAFETY NET ─────────────────────────────────────────────────────────
  // If we exceed the tilt limit while motors are live, latch them OFF
  // immediately. This requires an explicit "M" re-enable to recover (no
  // auto-recovery), so a fall can't keep driving the wheels into the ground or
  // a person's hand.
  ...
  // 32-bit aligned reads are atomic on Cortex-M3, so no need to disable
  // interrupts.
  long encL = encoderLeft;
  long encR = encoderRight;
  float wheelVelocity =
      ((float)((encL - prevEncoderLeft) + (encR - prevEncoderRight)) * 0.5) /
      dt;
  ...
  // Prevent Integral Windup: only build integral while motors are actually
  // running.
  if (!motorsEnabled) {
    integral = 0.0;
    prevError =
        error; // Prevents massive derivative spike when turning motors back on
  } else {
    integral += error * dt;
  }
  ...
  // Velocity damping: subtract Kd_vel * wheelVelocity before it reaches the
  // motors.
  ...
  // Leg servo torque/position heartbeat — non-blocking, self-timed off
  // millis(), fires at most once per 1500ms per servo regardless of the 100Hz
  // loop rate above.
  healLegServos();

  // Data log (Print at 20Hz so we don't saturate the serial bus).
  // ORIGINAL fields (PITCH/PID_OUT/ENC_L/ENC_R) kept in order; new fields
  // appended.
  if (now - lastPrintTime >= 50000) {
    ...
    Serial1.print(", ");
    Serial1.print("ENC_L:");
    Serial1.print(encoderLeft);
    Serial1.print(", ");
    Serial1.print("ENC_R:");
    Serial1.print(encoderRight);
    Serial1.print(", ");
    Serial1.print("VEL:");
    Serial1.print(wheelVelocity);
    Serial1.print(", ");
    Serial1.print("VEL_MMS:");
    Serial1.print(linearVelMmS, 2);
    Serial1.print(", ");
    Serial1.print("VEL_MS:");
    Serial1.print(linearVelMS, 4);
    Serial1.print(", ");
    Serial1.print("KDVEL:");
    Serial1.print(Kd_vel, 4);
    Serial1.print(", ");
